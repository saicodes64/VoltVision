...
const DataUpload = ({ onUploadSuccess }: { onUploadSuccess: () => void }) => {
    ...
    const handleFile = async (file: File) => {
        ...
        try {
            ...
            onUploadSuccess();  // Refresh all dashboard data
        } catch (err: any) {
    ...
    const downloadSampleCSV = () => {
        // Generate an inline sample CSV with realistic data
        const now = new Date();
        ...
        <div className="card-gradient rounded-xl border border-border p-5">
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">CSV with timestamp + usage columns</p>
                    </div>
                </div>
                <button
                ...
                >
                    ...
                    <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Columns: <code className="bg-muted/50 rounded px-1">timestamp</code> + <code className="bg-muted/50 rounded px-1">usage_kwh</code> (or similar)
                        </p>
                    </div>
                </div>
            )}
            ...
            {uploading && (
                <div className="rounded-xl border border-primary/20 bg-primary/5 py-6 flex flex-col items-center gap-3 animate-pulse">
                    ...
                    <p className="text-xs text-muted-foreground">Cleaning, analyzing and forecasting your usage</p>
                </div>
            )}
                ...
                <div className="rounded-xl border border-peak/20 bg-peak/5 p-4">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            <p className="text-sm font-semibold text-peak">Upload Failed</p>
