...
def get_latest_24h_data(user_id: str) -> List[Dict]:
    ...
    try:
        # Sort descending to get latest, limit 24, then reverse to chronological order
        cursor = energy_collection.find({"user_id": user_id}, {"_id": 0}).sort("timestamp", -1).limit(24)
